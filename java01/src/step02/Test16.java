 ...
 * => 그래서 주로 인스턴스를 사용하기 전에 그 메모리를 초기화시키는 용도로 사용.
...
public class Test16 {
  ...
  static class Student {
    ...
    Student(String name, int kor, int eng, int math) {
      ...
      // 생성자를 호출할 때 그 인스턴스의 주소를 담고있는 변수.
      // 인스턴스의 필드를 가리킬 때 내장변수 this를 사용하라.
      this.name = name;
  ...
  public static void main(String[] args) {
     ...
     //2) Student 클래스에 정의된 메서드 중에서 다음 4개의 값을 받을 수 있는 생성자를 호출하라.
     ...
     * => 클래스와 관련된 코드는 그 클래스와 묶어 두는 것이 유지보수에 좋다.
     * => 나중에 클래스 별로 별개의 파일로 분리할 때 유리하다.
     *      유지보수 입장에서도 코드를 관리하기 쉽다.
     * */  
    
    //printStudent() 메서드 또한 이 메서드와 관련된 클래스 안에 옮겨 둔다.
    for(int i =0; i<arr1.length ; i++)
    {
     Student.printStudent(arr1[i]); // 좀 더 어떤 클래스의 메서드인지 다루기가 쉬워짐.
    }
