/*static nested class 문법을 이요하여 상수 값을 계층 구조로 관리한다.
 * => CWork. CUserType, .. 클래스처럼
 *      각 클래스마다 상수를 개별적으로 관리하는 방법보다,
 *      다음과 같이 한 클래스 안에 nested클래스로 상수를 묶어서 관리하면
 *      상수를 사용하기 편해진다.
 *      특히 계층 구조 형식을 취하기 때문에 
 *      OGNL(ㅇㅖ: int a   = aa.bb.vv.cc;) 표기법처럼 상수를 사용할 수 있다.
 * */
